...
class Vote(Base):
    ...
    rankings_json: Mapped[str] = mapped_column(Text, nullable=False)  # JSON: [itinerary_id, ...]
    submitted_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=lambda: datetime.now(UTC)
    )

    participant: Mapped[Participant] = relationship("Participant", back_populates="votes")  # type: ignore[name-defined]
