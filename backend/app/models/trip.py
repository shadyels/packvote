...
class Trip(Base):
    ...
    participants: Mapped[list[Participant]] = relationship(
        "Participant", back_populates="trip"
    )  # type: ignore[name-defined]
    itineraries: Mapped[list[Itinerary]] = relationship(
        "Itinerary", back_populates="trip", foreign_keys="Itinerary.trip_id"
    )  # type: ignore[name-defined]
    votes: Mapped[list[Vote]] = relationship("Vote", back_populates="trip")  # type: ignore[name-defined]
    vote_rounds: Mapped[list[VoteRound]] = relationship(
        "VoteRound", back_populates="trip"
    )  # type: ignore[name-defined]
