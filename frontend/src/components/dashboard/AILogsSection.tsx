      ...
      <Table>
        <TableHeader>
          <TableRow className="border-border hover:bg-transparent">
            <TableHead className="text-black/50">When</TableHead>
            <TableHead className="text-black/50">Model</TableHead>
            <TableHead className="text-black/50">Provider</TableHead>
            <TableHead className="text-black/50 text-right">
              Latency (ms)
            </TableHead>
            <TableHead className="text-black/50 text-right">Tokens in</TableHead>
            <TableHead className="text-black/50 text-right">
              Tokens out
            </TableHead>
            <TableHead className="text-black/50 text-center">Valid</TableHead>
            <TableHead className="text-black/50">Error</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {logs.map((log) => (
            <TableRow key={log.id} className="border-border hover:bg-muted/20">
              <TableCell className="text-black/60 text-xs whitespace-nowrap">
                {new Date(log.created_at).toLocaleString()}
              </TableCell>
              <TableCell className="text-black text-xs font-mono">
                {log.model_used}
              </TableCell>
              <TableCell className="text-black/60 text-xs capitalize">
                {log.provider}
              </TableCell>
              <TableCell className="text-right text-black/60 text-xs">
